...
#### Libib Import Folder Registration (tentative v0.10.0 PR3)

Libib exports conventionally live in one operational audit-area directory below
`input/libib/`. A tentative `inventory_import_folders` repository may register
`folder_id`, relative `folder_path`, exact `expected_collection_label`, first and
last successful import times, and notes. This is durable operational safety
metadata, not inventory or source evidence.
